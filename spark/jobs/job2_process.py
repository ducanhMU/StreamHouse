"""
Spark Job 2: Silver to Gold Layer Processing (Production Stable)
-----------------------------------------------------------------------
FIX: "NULL value violates not-null constraint"
SOLUTION: Applied F.coalesce() to ALL columns to ensure default values.
-----------------------------------------------------------------------
"""
...
# --- SIMULATION SETTING ---
SIMULATION_MODE = False # Set to True to enable data limiting for testing
...
def read_silver(spark, table_name):
    ...
    if SIMULATION_MODE:
        logger.warning(f"⚠️ [SIMULATION] Limiting '{table_name}' to latest {SIMULATION_LIMIT} rows.")
...
def deduplicate_for_upsert(df, primary_keys, time_col="event_time"):
    window = Window.partitionBy(primary_keys).orderBy(F.col(time_col).desc())
...
def write_gold(df, table_name, primary_keys):
    ...
    try:
        df = deduplicate_for_upsert(df, primary_keys)
        ...
        df = df.repartition(1 if count < 1000 else 4)
        df.cache()
...
# ==========================================
# PROCESSING LOGIC (WITH DEFAULTS)
# ==========================================
...
def process_effective_unit_strength(spark):
    ...
    try:
        ...
        wind = F.coalesce(F.col("we.wind_speed_kmh"), F.lit(0.0)).cast(DoubleType())
        precip = F.coalesce(F.col("we.precipitation_mm"), F.lit(0.0)).cast(DoubleType())
        intensity = F.coalesce(F.col("we.intensity"), F.lit(0.0)).cast(DoubleType())
        ...
        # APPLY COALESCE TO ALL OUTPUT COLUMNS
        gold_df = df.select(
            F.col("s.unit_id"),
            F.col("s.region_id"),
            F.col("w.id").alias("weapon_id"),
            F.coalesce(F.col("terrain_factor"), F.lit(1.0)).alias("terrain_factor"),
            F.coalesce(F.col("effective_range_km"), F.lit(0.0)).alias("effective_range_km"),
            F.coalesce(F.col("adjusted_hit_probability"), F.lit(0.0)).alias("adjusted_hit_probability"),
            F.coalesce(F.col("attacking_strength"), F.lit(0.0)).alias("attacking_strength"),
            F.coalesce(F.col("s.event_time"), F.current_timestamp()).alias("event_time")
        ).filter(
...
def process_threat_assessment(spark):
    ...
    try:
        ...
        gold_df = df.select(
            ...
            F.col("d.region_id"),
            F.coalesce(F.col("t.iff_status"), F.lit("unknown")).alias("iff_status"),
            F.coalesce(F.col("adjusted_confidence"), F.lit(0.0)).alias("adjusted_confidence"),
            F.coalesce(F.col("predicted_threat"), F.lit("medium")).alias("predicted_threat"),
            F.coalesce(F.col("distance_km"), F.lit(0.0)).alias("distance_km"),
            F.coalesce(F.col("response_time_sec"), F.lit(0.0)).alias("response_time_sec"),
            F.coalesce(F.col("d.event_time"), F.current_timestamp()).alias("event_time")
        ).filter(
...
def process_alerts_with_commands(spark):
    ...
    try:
        ...
        df = df.withColumn("action", 
            F.when(F.col("u.role") == "commander", F.col("c.action")).otherwise(F.lit("none"))
        ...
        gold_df = df.select(
            ...
            F.col("d.region_id"),
            F.coalesce(F.col("a.threat_level"), F.lit("low")).alias("threat_level"),
            F.col("c.id").alias("command_id"),
            F.coalesce(F.col("action"), F.lit("none")).alias("action"),
            F.col("u.id").alias("user_id"),
            F.coalesce(F.col("final_event_time"), F.current_timestamp()).alias("event_time")
        ).filter(
...
def process_logistics_readiness(spark):
    ...
    try:
        ...
        gold_df = df.select(
            F.col("s.unit_id"),
            F.col("u.region_id"),
            F.coalesce(F.col("s.supply_level"), F.lit(0.0)).alias("supply_level"),
            F.coalesce(F.col("projected_supply"), F.lit(0.0)).alias("projected_supply"),
            F.coalesce(F.col("resupply_feasibility"), F.lit(0)).alias("resupply_feasibility"),
            F.coalesce(F.col("final_event_time"), F.current_timestamp()).alias("event_time")
        ).filter(
            F.col("unit_id").isNotNull() & 
            F.col("region_id").isNotNull()
...
def process_engagement_analysis(spark):
    ...
    try:
        ...
        # APPLY COALESCE TO ALL OUTPUT COLUMNS
        # Default result to "missed" if null
        gold_df = df_final.select(
            ...
            F.coalesce(F.col("e.result"), F.lit("missed")).alias("result"),
            F.coalesce(F.col("adjusted_hit_probability"), F.lit(0.0)).alias("adjusted_hit_probability"),
            F.coalesce(F.col("impact_factor"), F.lit(1.0)).alias("impact_factor"),
            F.coalesce(F.col("e.event_time"), F.current_timestamp()).alias("event_time")
        ).filter(
...
def main():
    ...
    try:
        while True:
            ...
            logger.info(f"--- STARTING BATCH CYCLE AT {time.strftime('%H:%M:%S')} ---")
